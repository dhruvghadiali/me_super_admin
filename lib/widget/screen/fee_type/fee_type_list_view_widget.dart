...
class FeeTypeListViewWidget extends StatelessWidget {
  const FeeTypeListViewWidget({super.key, required this.onRefresh, required this.feeTypes});

  final Function onRefresh;
  final List<FeeType> feeTypes;

  Future<void> deleteFeeType({required BuildContext context, required FeeType feeType}) async {
  ...
  Future<void> editFeeType({required BuildContext context, required FeeType feeType}) async {
    final FeeTypeController feeTypeController = Get.put(FeeTypeController());
    feeTypeController.setFeeTypeForm(feeType);
  }

  void onAddFeeTypeClicked(BuildContext context) {
    final FeeTypeController feeTypeController = Get.put(FeeTypeController());
    feeTypeController.resetFeeTypeForm();
    Navigator.pushNamed(context, RoutePaths.feeTypeForm);
  }
          ...
          children: [
                      ...
                      endActionPane: ActionPane(
                        ...
                        children: [
                          EditSlidableActionWidget(
                            onEdit: () => editFeeType(context: context, feeType: feeTypes[index]),
                          ),
                          DeleteSlidableActionWidget(
                            onDelete:
                                () => deleteFeeType(context: context, feeType: feeTypes[index]),
                          ),
                        ],
                      ),
            ...
            Positioned(
              ...
              child: FloatingActionButton(
                onPressed: () => onAddFeeTypeClicked(context),
