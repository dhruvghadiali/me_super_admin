...
class StateSelectionFormWidget extends StatelessWidget {
  const StateSelectionFormWidget({super.key, required this.formFieldKey, required this.selectedState, required this.onChange, required this.validator});
    ...
    return GetBuilder<StateController>(
      builder: (stateControllerContext) {
        return DropdownWidget(
          ...
          selectedItem: stateControllerContext.states.isEmpty ? '' : selectedState.id,
          appColorScheme: AppColorScheme.primary,
          onChanged: (String value) => onStateChange(value, stateControllerContext.states),
          items: stateControllerContext.states.isEmpty ? [] : stateControllerContext.states.map((state) => {'label': state.name, "value": state.id}).toList(),
        );
      },
    );
  }
